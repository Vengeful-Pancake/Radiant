...
def camera():
    ...
    while True:
        ...
        fgmask2 = backgroundObject.apply(frame1)
        ...
        countors2, _ = cv2.findContours(fgmask2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        frameCopy1 = frame0.copy()
        frameCopy2 = frame1.copy()

        # loop inside the countor and search for bigger ones

        for cnt in countors1:
            if cv2.contourArea(cnt) > 20000:
                ...
                # write a text near the object 
                cv2.putText(frameCopy1 ,"Motion detected", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.3 , (0,255,0), 1, cv2.LINE_AA)

        for cnt in countors2:
            if cv2.contourArea(cnt) > 20000:
                ...
                # draw a rectangle around the area
                cv2.rectangle(frameCopy2, (x,y), (x+width, y+ height) , (0,0,255), 2)

                # write a text near the object 
                cv2.putText(frameCopy2 ,"Motion detected", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.3 , (0,255,0), 1, cv2.LINE_AA)    

        forground1 = cv2.bitwise_and(frame0,frame0 , mask = fgmask1)
        forground2 = cv2.bitwise_and(frame1,frame1 , mask = fgmask2)
        stacked = np.hstack((frame,forground1,forground2,frameCopy1,frameCopy2))
